...
<<<<<<< HEAD
			while ( count < 500);// here is the problem >>>> The program is not able to decide when to stop the training even after the clearity in the approach!!!!!!!  What so ever you do there is a number which is to be inserted by a human to control the no of  training cycles.
=======
...
<<<<<<< HEAD
/*Story Ahead ...... today as on 4/4/2013

I have finally achieved (with somewhat compromises with expectations) what I wanted to achieve in terms of the capacity of the program.  Still there is a lot to done. But I am happy with the progress over the years in the program.  
Today we have an utility which is ONLINE ! It is able to produce the graphs for error so that one can decide the minimum number of cycle for which the network is to be trained to achieve maximum accuracy in function approximation and to avoid over training of the network.
In next version for development we can make the program capable enough to terminate the training of the network.

Still I wish to develop this as an GUI application but that is a long time goal.  As if for now I will be happy with one Research Paper on this subject !!!
...
<<<<<<< HEAD
	public void testTESTSET( int cycles) throws IOException {
		PrintWriter pr = new PrintWriter( new FileWriter("TESTSETtry"+cycles+"srmt1.txt"));	
=======
	public void testTESTSET() throws IOException {
		PrintWriter pr = new PrintWriter( new FileWriter("TESTSETtry500srmt1.txt"));	
>>>>>>> 59dd52e3b873b3712e2aa700f53a98ceca271d3e
	...
	public static void main (String[] args ) throws IOException {
		...
		//m1.printWeights();		
<<<<<<< HEAD
		FileWriter fw = new FileWriter( "train" +cycles +"srmt1.txt");
=======
		FileWriter fw = new FileWriter( "train500srmt1.txt");
>>>>>>> 59dd52e3b873b3712e2aa700f53a98ceca271d3e
		PrintWriter pw = new PrintWriter( fw);
		int k =1;	
		...
		System.out.println(" Reading and testing the final weights ");
<<<<<<< HEAD
		m1.saveWeights("weightsFilesrm"+cycles);
		m1.readWeights("weightsFilesrm"+cycles);				
		m1.feedForward();
		int choice = 0;
		m1.test();
		m1.testTESTSET(cycles);
=======
		m1.saveWeights("weightsFilesrm500");
		m1.readWeights("weightsFilesrm500");				
		m1.feedForward();
		int choice = 0;
		m1.test();
		m1.testTESTSET();
